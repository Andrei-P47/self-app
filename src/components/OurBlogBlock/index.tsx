...
// import { CarouselProvider, Sliders, Slide, ButtonBack, ButtonNext } from 'pure-react-carousel';
...
const OurBlogBlock: NextPage = () => {
    ...
    return (
        <div className="container px-4 xs:px-0 xs:mx-auto w-[360px] xs:w-[1110px] ">
            {/*{isBreakpoint ? (*/}
            {/*<CarouselProvider className="items-center justify-center w-full h-full py-[50px] xs:py-[120px]" naturalSlideWidth={100} isIntrinsicHeight={true} totalSlides={6} visibleSlides={3} step={1} infinite={false} dragEnabled={!isBreakpoint} touchEnabled={!isBreakpoint}>*/}
            ...
            {/*                Our blog*/}
            {/*            </div>*/}
            {/*            <div className="xs:block hidden flex flex-row gap-[15px] ">*/}
            ...
            {/*            <Sliders>*/}
            {/*                <div id="slider" className="h-full flex flex-col xs:flex-row items-start gap-[20px] xs:gap-30px transition ease-out duration-700">*/}
            {/*                    {TestimonialsCards.map(({ id,icon,date, title, description}, index) => {*/}
            {/*                        // if(isBreakpoint && index < 3){*/}
            {/*                            return (*/}
