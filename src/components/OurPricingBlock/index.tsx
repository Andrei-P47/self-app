...
const OurPricingBlock: NextPage = () => {
    ...
    return (
        ...
        <div className="container pl-4 xs:px-0 xs:mx-auto w-[360px] xs:w-[1110px] my-[50px] xs:my-[120px] grid gap-[30px] xs:gap-[50px] ">
            ...
            <div className="flex flex-row lastBlockFontWeight gap-[36px] xs:gap-30px overflow-visible">


                {OurPricingCards.map((service, index) => {
                    if(isBreakpoint ?  index < 1 : index < OurPricingCards.length) {
                        return (

                    <div className="min-h-[525px] w-[272px] xs:w-[255px] flex flex-col items-start py-[35px] px-[20px] gap-[30px] bg-white group hover:bg-primary rounded-md shadow-card-3" key={index}>
                        <div className="flex flex-col gap-[16px] w-full">
                            <div className="headline_6_Bold group-hover:text-white">{service.title}</div>
                            <div className="flex flex-row items-center justify-between gap-[89px] xs:gap-0">
